...
if (!function_exists('render_paginate')) {
    ...
    {
        $page_num = isset($_GET['page']) && !empty(request('page')) ? request('page') : 1;
        $desactive = $page_num == 1 ? 'disabled' : '';
        $html = '<ul class="pagination" dir="ltr">';
        $html .= ' <li class="page-item ' . $desactive . '">
        ...
        $n_desactive = $page_num == $total_page ? 'disabled' : '';
        $html .= ' <li class="page-item ' . $n_desactive . '">
